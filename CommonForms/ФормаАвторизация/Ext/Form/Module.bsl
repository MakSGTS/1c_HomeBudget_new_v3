...
Процедура РегстрацияКоманда(Команда)
	...
	ТекЛогин = ОбщиеПроцедурыИФункцииСервер.ПолучитьУчетнуюЗапись();
	Если ТекЛогин <> Неопределено Тогда
		ТекстСообщ = СтрШаблон("Вы уже зарегестрированы и авторизированы под учётной записью %1", ТекЛогин);
		ДиалогиСПользователемКлиент.ПоказатьСообщениеПользователю(ТекстСообщ);
		Возврат;
	КонецЕсли;
