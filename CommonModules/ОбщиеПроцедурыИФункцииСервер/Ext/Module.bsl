 ...
 Функция ЗаданиеВыполнено(ПроверяемоеСобытие) Экспорт
	...
	Если ФонЗад <> Неопределено Тогда
		...
		ИначеЕсли ФонЗад.Состояние = СостояниеФоновогоЗадания.ЗавершеноАварийно Тогда
			...
			ОбщиеПроцедурыИФункцииСервер.ЗаписатьЛоги(ТекстОшибки);
		ИначеЕсли ФонЗад.Состояние = СостояниеФоновогоЗадания.Отменено Тогда
			...
			ОбщиеПроцедурыИФункцииСервер.ЗаписатьЛоги("Отмена фонового задания: " + ИмяФона); 
		КонецЕсли;
	КонецЕсли;
...
#Область Логи
Процедура ЗаписатьЛоги(ТекстОшибки, СтруктураДанных = Неопределено, ОтправитьНаСервер = Истина, Важность = Неопределено) Экспорт
