...
Функция ОтпарвитьДанныеНаСервер(АдресВХ, УчетнаяЗапись, Данные = Неопределено, ПоказатьСообщение = Истина, ТелоСтрокиСтатистика = "") Экспорт
	...
	УчетнаяЗапись = ОбщиеПроцедурыИФункцииСервер.ПолучитьУчетнуюЗапись();//TODO: повторный запрос!!!!. Заявлен передаваемый параметр УчетнаяЗапись!
	...
	АдресСкрипта = АдресСкрипта + УчетнаяЗапись.Логин;
	...
	ПараметрыПодключения.ИмяПользователя = УчетнаяЗапись.Логин;
	ПараметрыПодключения.Пароль = УчетнаяЗапись.Пароль;
