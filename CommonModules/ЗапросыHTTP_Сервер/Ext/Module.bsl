...
Функция ОтпарвитьДанныеНаСервер(Путь, ИмяПользователя, Пароль, Данные = Неопределено, ПоказатьСообщение = Истина, ТелоСтрокиСтатистика = "") Экспорт
	...
	АдресСкрипта = АдресСкрипта + ИмяПользователя;
...
Процедура ПолучитьДанныеССервера(Путь = "", ИмяПользователя, Пароль) Экспорт
	//TODO: ssl = Новый ЗащищенноеСоединениеOpenSSL;
	...
	ОтветHTTP = ВыполнитьHTTTP_Запрос(ПараметрыПодключения, ПараметрыЗапроса); //TODO Обработать ОтветHTTP = Неопределено 	
	Если ОтветHTTP.КодСостояния = 200 Тогда
	...
	Иначе
		ЛогированиеОшибокСервер.ЗаписатьОшибкуВЛоги("Ошибка получения данных. " + Строка(ОтветHTTP.КодСостояния));
	КонецЕсли;
КонецПроцедуры
