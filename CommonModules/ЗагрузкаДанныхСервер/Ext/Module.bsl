...
Процедура ЗагрузитьДанные(Путь) Экспорт 
	...
	Попытка
		...
		//ЧтениеXML.ОткрытьФайл(ПутьXML);
		НоваяЗагрузка_XML(ПутьXML);
		
//		Попытка
//			УдалитьФайлы(ПутьXML);
//		Исключение
//			Ошибка = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
//			ЛогированиеОшибокСервер.ЗаписатьОшибкуВЛоги("Не удалость удалить файл." + Символы.ПС + Ошибка);
//		КонецПопытки;
	...
	Исключение
		Ошибка = "Ошибка загрузки бэкапа из файла." + Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
	...
	Исключение
		...
		Текст = "Не удалось удалить файл." + Символы.ПС + ТекстОшибки;
...
//Новый вариант
Процедура НоваяЗагрузка_XML(Путь) Экспорт
	ЧтениеXML = Новый ЧтениеXML;
	...
	Пока ЧтениеXML.Прочитать() Цикл
		Пока ВозможностьЧтенияXML(ЧтениеXML) Цикл
			...
			Если СтрНайти(Объект.Метаданные().ПолноеИмя(), "Справочник") > 0 Тогда
				Если Объект.Предопределенный Тогда
					...
					Если ЗначениеЗаполнено(СпрСс) Тогда 
						...
						КонецЕсли;
						guid = ПолучитьGUID(Путь, ИмяОбъекта, Объект.Код, ТипПриведения);
			...
			ИначеЕсли СтрНайти(Объект.Метаданные().ПолноеИмя(), "Документ") > 0 Тогда
				...
				КонецЕсли;
			ИначеЕсли СтрНайти(Объект.Метаданные().ПолноеИмя(), "Регистр") > 0 Тогда
				Объект.Записать();
			ИначеЕсли СтрНайти(Объект.Метаданные().ПолноеИмя(), "Константа") > 0 Тогда
				Объект.Записать();
			КонецЕсли;			
		КонецЦикла;
	КонецЦикла;
