...
Процедура ОбновитьДанныеНаГлавномЭкране(Объект = Неопределено) Экспорт
	ЗаполнитьШкалуБюджета(Объект);
	ЗаполнитьГрафикРасходов(Объект);
	ЗаполнитьДиаграммуДенежныеСредства(Объект)	
КонецПроцедуры

Процедура ЗаполнитьШкалуБюджета(Объект) Экспорт
...
Процедура ЗаполнитьГрафикРасходов(Объект) Экспорт
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
...
Процедура ЗаполнитьДиаграммуДенежныеСредства(Объект) Экспорт
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
