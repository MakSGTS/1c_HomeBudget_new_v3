...
Функция ВыгрузитьДанныеСервер(Путь_тмп) Экспорт
	...
	Ответ = ПодготовитьФайлБэкапа(Путь_тмп, Ложь);
	...
	Если Ответ.ФайлСоздан Тогда
		...
		Возврат ЗапросыHTTP_Сервер.ОтпарвитьДанныеНаСервер(Ответ.АдресВХ, , , ТелоСтрокиСтатистика);
	Иначе
		Возврат Ложь;
	КонецЕсли;
КонецФункции

Функция ПодготовитьФайлБэкапа(Путь, ВФайл = Ложь) Экспорт
	Возврат ЗаписатьДанныеВФайл(Путь, ВФайл);
КонецФункции

Функция ЗаписатьДанныеВФайл(Путь, ВФайл) Экспорт
