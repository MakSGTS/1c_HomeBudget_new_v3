...
Процедура ПриЗаписиНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	Если Объект.УстановитьНачальныйОстаток И Элементы.НачальныйОстаток.Доступность Тогда 
		Если Объект.НачальныйОстаток = 0 Тогда 
			...
			Отказ = Истина;
		Иначе
			//Если Модифицированность Тогда  //TODO
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если Отказ Тогда
		ДиалогиСПользователемКлиент.ПоказатьСообщениеПользователю("Счёт с таким наименованием уже есть!");
	КонецЕсли;
КонецПроцедуры

&НаСервереБезКонтекста
Процедура ПроверитьДубльНаименованияНаСервере(Отказ, Ссылка, Наименование)
	Отказ = ОбщиеПроцедурыИФункцииСервер.ПроверитьНаличиеДубляНаименования(Наименование, "Счета", Ссылка); 
КонецПроцедуры
